...
import { ShoppingCart, Truck, DollarSign, Clock, CheckCircle, XCircle, Package, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {(stats?.pendingOrders ?? 0) > 0 && (
        <Link
          ...
          className="flex items-center gap-3 p-4 bg-yellow-50 border border-yellow-300 rounded-xl hover:bg-yellow-100 transition-colors"
        >
          <AlertTriangle className="h-5 w-5 text-yellow-600 flex-shrink-0" />
          <div className="flex-1">
            <p className="font-semibold text-yellow-800">
              {stats.pendingOrders} order{stats.pendingOrders > 1 ? 's' : ''} waiting for your approval
            </p>
            <p className="text-xs text-yellow-600 mt-0.5">Click here to review and approve or reject pending orders</p>
          </div>
          <span className="text-xs font-semibold text-yellow-700 bg-yellow-100 border border-yellow-300 px-3 py-1 rounded-full">
            Review →
          </span>
        </Link>
      )}
