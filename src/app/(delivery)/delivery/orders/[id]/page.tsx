...
export default function DeliveryOrderDetailPage() {
  ...
  const fetchOrder = () => {
    api.get(`/orders/${id}`).then(({ data }) => setOrder(data.data)).catch(() => router.push('/delivery/orders')).finally(() => setLoading(false));
  };
        ...
        <div className="card p-6 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {site.geoLocation?.lat ? (
              ...
              <a
                href={`https://maps.google.com/?q=${encodeURIComponent([site.address, site.city, site.state, site.pincode].filter(Boolean).join(', '))}`}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-secondary flex items-center gap-1 text-sm"
              >
