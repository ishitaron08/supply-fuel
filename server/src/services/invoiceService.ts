...
export const generateInvoice = async (orderId: string): Promise<string | null> => {
  try {
    ...
    doc
      .fillColor('#333')
      .text(`Diesel (${order.fuelType.toUpperCase()})`, 60, rowTop)
      .text(order.quantityLiters.toLocaleString('en-IN'), 250, rowTop)
      .text(`₹${order.pricePerLiter.toFixed(2)}`, 330, rowTop)
      .text(`₹${baseAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 440, rowTop);
    ...
    doc
      ...
      .text(`₹${baseAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 440, totalsTop + 10)
      .text(`GST (${order.gstPercentage}%):`, 350, totalsTop + 28)
      .text(`₹${order.gstAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 440, totalsTop + 28);
    ...
    doc
      ...
      .text(`₹${order.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 430, totalsTop + 55);

    // Payment info
    doc
      .fontSize(10)
      .fillColor('#333')
      .text(`Payment Mode: ${order.paymentMode.toUpperCase()}`, 50, totalsTop + 55)
      .text(`Payment Status: ${order.paymentStatus.toUpperCase()}`, 50, totalsTop + 70);

    // Footer
    doc
      .fontSize(8)
      .fillColor('#999')
      .text('This is a computer-generated invoice and does not require a signature.', 50, 700, {
        align: 'center',
      });
