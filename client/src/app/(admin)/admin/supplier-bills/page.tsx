                  ...
                  {bills.map((bill: any) => (
                    <tr key={bill._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-4">
                        {bill.billFileUrl && (
                          <a
                            href={`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${bill.billFileUrl}`}
                            target="_blank"
...
function UploadBillModal({ open, onClose, onUploaded }: { open: boolean; onClose: () => void; onUploaded: () => void }) {
  ...
  useEffect(() => {
    if (open) {
      api.get('/orders?status=delivered&limit=100').then(({ data }) => setOrders(data.orders || [])).catch(() => {});
    }
  }, [open]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file || !orderId || !vendorName || !billNumber || !amount) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <select value={orderId} onChange={(e) => setOrderId(e.target.value)} className="input-field">
            ...
            {orders.map((o: any) => (
              <option key={o._id} value={o._id}>{o.orderNumber} — {o.customerId?.name}</option>
            ))}
          </select>
        </div>
