...
export default function AdminOrderDetailPage() {
  ...
  const fetchOrder = () => {
    api.get(`/orders/${id}`).then(({ data }) => setOrder(data.order)).catch(() => router.push('/admin/orders')).finally(() => setLoading(false));
  };
  ...
  const handleReject = async () => {
    ...
    setActing(true);
    try {
      await api.put(`/orders/${id}/reject`, { rejectionReason });
  ...
  const openAssignModal = async () => {
    try {
      const [p, v] = await Promise.all([api.get('/users/delivery-partners'), api.get('/vehicles')]);
      setPartners(p.data.partners || []);
      setVehicles(v.data.vehicles || []);
      setAssignOpen(true);
    } catch {
      toast.error('Failed to load partners/vehicles');
    }
  };
            ...
            onClick={async () => {
              try {
                ...
                if (data.invoice?.pdfUrl) {
                  window.open(`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${data.invoice.pdfUrl}`, '_blank');
                }
              } catch {
                try {
      ...
      <Modal open={assignOpen} onClose={() => setAssignOpen(false)} title="Assign Delivery">
        <div className="space-y-4">
          <div>
            ...
            <select value={selectedPartner} onChange={(e) => setSelectedPartner(e.target.value)} className="input-field">
